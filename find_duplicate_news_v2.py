...
test=pd.read_csv('test_news.csv', encoding='utf-8')
...
test_vector=[]
for news in test.news_title:
	test_vector.append(sent_to_vec(news, google_model))
...
for i in range(len(test_vector)):
    sim_list=cosine_similarity(train_vector,test_vector[i].reshape(1,-1))
    score_list= sorted(enumerate(sim_list), key=operator.itemgetter(1),reverse=True)[:1]
    if score_list[0][1] > 0.8:
        duplicate.append("yes")
        indexx.append(score_list[0][0])
        score.append(score_list[0][1])
    else:
        duplicate.append("no")
        indexx.append('-1')
        score.append(0)
